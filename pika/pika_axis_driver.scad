// pika_axis_driver.scad - RepRapMicron motion stage
// (c)2025 vik@diamondage.co.nz, released under the terms of the GPL V3 or later
// One PIKA Axis Diver. Three are needed for the complete PIKA mechanism
// Uses a pantograph-like mechanism, paralleled up to provide stable
// drive, anchor, and effector ends.
//
// 07-May-2026
//
// Branched from the MAUS axis driver. Now has PIKA sizings and versioning by default,
// and experimental mounting points for a NEMA14 motor.
//
...
mb_length_in_holes=2;       // Set this to 3 for MAUS
...
module nema17_plate(nema_plate_thick=5,nema_corner_rad=2) difference() {
   ...
   // Screw holes for NEMA14/NEMA17
    nema14_screw_sep=26;
    for (i=[0:3]) rotate([0,0,90*i]) hull() {
        translate([nema17_screw_sep/2,nema17_screw_sep/2,0]) cylinder(h=nema_plate_thick*3,r=m3_screw_rad,$fn=16,center=true);
        translate([nema14_screw_sep/2,nema14_screw_sep/2,0]) cylinder(h=nema_plate_thick*3,r=m3_screw_rad,$fn=16,center=true);
    }
     // Collar hole
