...
def emit_gcode(f, img, config):
    ...
    f.write(f"G1 Z{config.safe_z:.3f} F{FEED_Z} ; Move to safe Z height\n\n")
    ...
    for y in range(height):
        for x in range(width):
            ...
            if condition:
                ...
                f.write(f"G1 Z{config.safe_z:.3f} F{FEED_Z} ; Safe Z\n")
                f.write(f"G1 X{gx:.2f} Y{gy:.3f} F{FEED_MOVE} ; Move to pixel\n")
                # Move down to just above surface
                f.write(f"G1 Z{config.draw_z+10:.3f} F{FEED_Z} ; Pen down in two stages\n")
                # Lightly touch down
                f.write(f"G1 Z{config.draw_z:.3f} F{SLOW_FEED_Z}\n")
                # Lightly lift off
                f.write(f"G1 Z{config.draw_z+10:.3f} F{SLOW_FEED_Z} ; Pen up in two stages\n")
                # Rush up to safe Z height like bat out of hell.
                f.write(f"G1 Z{config.safe_z:.3f} F{FEED_Z}\n")
